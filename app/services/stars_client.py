...
def get_people_for_unit(unit_id: str) -> list[Person]:
    ...
    # One unusable record must not cost the whole squadron, so skip it instead.
    people = []
    for record in records:
        try:
            people.append(Person(**record))
        except ValidationError:
            logger.warning("Skipping unusable STARS person %s", record.get("id"))

    # A posting that has ended is not on the unit; astimezone copes with no offset.
    now = datetime.now(UTC)
    current = [
        person
        for person in people
        if person.end_date is None or person.end_date.astimezone(UTC) > now
    ]

    logger.info(
        "Retrieved %d of %d people for unit %s", len(current), len(records), unit_id
    )
    return current
